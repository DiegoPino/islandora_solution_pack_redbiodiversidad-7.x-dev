/*
 * We are overriding a large part of the JS defined in leaflet (leaflet.drupal.js) and Leaflet_markercluster.drupal.js
 * We could clearall layers after markers where build by Leaflet_markercluster.drupal.js and the regroup them to be able to activate layer control on them
 * but it would hit perfomance
 * Not nice but it works, but we can't do otherwise beacuse leaflet_markercluster overrrides our implementation
 * Diego Pino Navarro 2014
 */
...
(function ($) {
  ...
  Drupal.behaviors.leaflet = { // overrides same behavior in leaflet/leaflet.drupal.js and Leaflet_markercluster.drupal.js
      ...
      $(settings.leaflet).each(function () {
        ...
        for (i = 0; i < this.features.length; i++) {
          ...
          if (feature.group) {
            ...
            if (cluster && clusterLayers[clusterGroup]) {
              lFeature.options.regions = feature.regions;
              clusterLayers[clusterGroup].addLayer(lFeature);//ok
			  markers_byclusterGroup[clusterGroup].push(lFeature);
		...
		//Also added so other can do with this groups later
		if (!clusterLayers)
			{
			$(document).trigger('leaflet.MarkerClusterGroup', [clusterGroup,clusterLayers[clusterGroup]]);
			}
		...
		for (var index in clusterLayers)
		 ...
		 // console.log(cats);
		  var controlCluster = L.control.layers(null, overlayMaps, {
		             collapsed: false,
		             position: 'topleft'
		         }
		     );
